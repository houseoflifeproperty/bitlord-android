...
public class AddTorrentViewModel extends AndroidViewModel
{
            ...
            try {
                switch (uri.getScheme()) {
                    ...
                    case Utils.CONTENT_PREFIX:
                        ...
                        break;
                    case Utils.TORRENTSTREAM_PREFIX:
                        v.mutableParams.setFromTorrentStream(true);
                        uri = uri.buildUpon().scheme(Utils.MAGNET_PREFIX).build();
    ...
    public boolean hasMediaFile()
    {
        ...
        for (int i = 0; i < infoObj.fileList.size(); i++) {
            String filePath = infoObj.fileList.get(i).getPath();
    ...
    public boolean addTorrent(boolean startStream) throws Exception
    {
        TorrentMetaInfo downloadInfo = info.get();
        if (downloadInfo == null)
            return false;
        ...
        AddTorrentParams params = new AddTorrentParams(source, fromMagnet, downloadInfo.sha1Hash,
                ...
                !mutableParams.isStartAfterAdd(), startStream);
