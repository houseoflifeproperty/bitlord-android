    ...
    implements Selectable<TorrentContentFileItem>
{
        ...
        implements ViewHolderWithDetails
    {
        ...
        void bind(TorrentContentFileItem item, ClickListener listener)
        {
            ...
            binding.fileOptionButton.setOnClickListener((v) -> {
                PopupMenu popup = new PopupMenu(MainApplication.getInstance().getApplicationContext(), v);
                popup.setOnMenuItemClickListener((menu_item) -> {
                    int itemID = menu_item.getItemId();
                    int position = getAdapterPosition();
                    if (listener != null && position >= 0) {
                        listener.onMenuItemClick(itemID, position, item);
                        return true;
                    }
                    return false;
                });
                popup.inflate(R.menu.detail_torrent_files_action_mode);

                Menu pMenu = popup.getMenu();
                if (!item.isFile) {
                    pMenu.findItem(R.id.share_stream_url_menu).setVisible(false);
                    pMenu.findItem(R.id.play_file_menu).setVisible(false);
                    pMenu.findItem(R.id.open_file_system_menu).setVisible(false);
                } else {
                    SettingsRepository pref = RepositoryHelper.getSettingsRepository(MainApplication.getInstance());
            ...
            if (isParentDir) {
                ...
                binding.fileOptionButton.setVisibility(View.GONE);
            } else {
    ...
    public interface ClickListener
    {
        ...
        boolean onMenuItemClick(int itemID, int position, @NonNull TorrentContentFileItem node);
    }
